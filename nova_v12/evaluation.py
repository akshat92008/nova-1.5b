...
def load_eval_cases(path: Path) -> list[dict[str, Any]]:
    ...
    for line, value in iter_jsonl(path):
        ...
        if not isinstance(files, dict) or not all(
            isinstance(key, str) and isinstance(content, str)
...
def aggregate_metrics(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    protocol_valid = sum(
        1
        for item in results
        if item["evidence"]["attempts"]
        and item["evidence"]["attempts"][0]["protocol_valid"]
    )
    patch_applied = sum(
        1
        for item in results
...
def release_decision(
    ...
    *,
    ...
    minimum_protocol_rate: float = 0.90,
    minimum_patch_application_rate: float = 0.90,
    ...
    gates = {
        ...
        "patch_application_rate": (
            c["patch_application_rate"] >= minimum_patch_application_rate
        ),
        ...
        "no_success_regression_vs_base": (
            c["task_success_rate"]
            >= b["task_success_rate"] - maximum_success_regression
        ),
    }
    report = {
...
def main() -> None:  # pragma: no cover - exercised by CLI smoke tests
    ...
    release.add_argument("--minimum-tasks", type=int, default=100)
